...
En el ejercicio número 4 (puntoCuatro), hicimos "algo" que representa un texto. Un texto normal, tipo documento. En ELM no existen clases, por lo tanto, creamos un tipo llamado Text y creamos funciones que trabajen sobre ese tipo. Como lo hicimos anteriormente, creamos el módulo que define la interfaz para el usuario. Después, creamos un tipo Text que es una lista de Strings y cada uno representa un párrafo. Creamos un texto vacío y la función insert position newParagraph (Text paragraphs) que es para crear párrafos. Como ELM no tiene listas mutables, lo que hace es crear uno nuevo y juntarlo con el anterior, ya que el original no se puede modificar. La función Delete es básicamente lo mismo pero en lugar de agregar párrafos, quita uno. Funciona igualmente que el anterior, creando una nueva lista por modificación que realizamos. La función display String.join "\n\n" paragraphs es para unir todos los párrafos con saltos de línea. 
Por último, en Elm no se pueden leer archivos directamente sin usar Cmd, entonces load y save se implementarían usando comandos para interactuar con archivos externos.
